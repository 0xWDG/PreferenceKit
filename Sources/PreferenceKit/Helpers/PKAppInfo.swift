...
public enum PKAppInfo {
    ...
    public static func appStoreInfo(_ forceRefresh: Bool = false) async -> PKAppInfoAppStoreInfo? {
        ...
        if let cached = UserDefaults.standard.data(forKey: "PKAppInfoAppStoreInfo"),
           let appStoreInfo = try? decoder.decode(PKAppInfoAppStoreInfo.self, from: cached),
           !forceRefresh {
            return appStoreInfo
        }
        ...
        guard
            let itunesURL = URL(
                string: "http://itunes.apple.com/lookup?bundleId=\(PKAppInfo.bundleIdentifier)"
            )
        else {
            return nil
        }
        ...
        do {
            ...
            UserDefaults.standard.set(data, forKey: "PKAppInfoAppStoreInfo")
            return try decoder.decode(PKAppInfoAppStoreInfo.self, from: data)
        } catch {
            print("Error")
        }
#endif
